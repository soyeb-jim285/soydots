...
Scope {
    ...
    // Keybinding cheatsheet data, read from the co-located keybindings.json
    // (generated by scripts/gen-keybindings.py). FileView hot-reloads it.
    property var keybindsData: []
    function _reloadKeybinds() {
        try {
            keybindsData = JSON.parse(_keybindsView.text())?.sections ?? [];
        } catch (e) {
            keybindsData = [];
        }
    }
    property var _keybindsView: FileView {
        path: Qt.resolvedUrl("keybindings.json")
        watchChanges: true
        blockLoading: false
        preload: true
        onTextChanged: root._reloadKeybinds()
    }
    ...
    readonly property var modeChips: {
        return [
            ...
            {
                text: (Config.launcherKeybindsPrefix || "k") + " keys",
                active: !!keybindsMatch
            }
        ];
    }
    ...
    readonly property string searchContextText: {
        ...
        if (keybindsMatch)
            return "Keybindings: Enter copies the chord. Scope with [domain], e.g. [zsh], [hypr], [tmux], [nvim].";
    ...
    function homeSuggestions() {
        ...
        if (Config.launcherKeybindsEnabled) {
            items.push({
                type: "suggestion",
                title: "Keybindings",
                subtitle: "Search hypr/zsh/tmux/nvim shortcuts; Enter copies the chord",
                query: (Config.launcherKeybindsPrefix || "k") + " volume",
                iconSource: "icons/IconKeyboard.qml",
    ...
    function resultKey(item) {
        ...
        if (item.type === "keybind") return "keybind:" + (item.keys ?? "") + ":" + (item.desc ?? "");
    ...
    function resultTitle(item) {
        ...
        if (item.type === "keybind") return item.desc ?? item.keys ?? "";
    ...
    function resultSubtitle(item) {
        ...
        if (item.type === "keybind") return item.group ?? "";
        return "";
    }
    ...
    function resultRepoLabel(item) {
        ...
        if (item.type === "keybind") return item.keys ?? "";
        return "";
    }
    ...
    function shortcutActions(item) {
        ...
        if (item.type === "app") {
            ...
            out.push({ label: "Enter copy chord", action: "activate" });
    ...
    function copyKeyChord(chord) {
        if (!chord) return;
        Quickshell.execDetached(["wl-copy", "--", chord]);
        root.closing = true;
    }
    ...
    // Keybindings prefix mode — fully in-memory (no Process/Timer), follows
    // the emoji/clipboard model. Requires a whitespace boundary after the
    // prefix so app names starting with the prefix char don't trigger it.
    property var keybindsMatch: {
        if (!Config.launcherKeybindsEnabled) return null;
        let prefix = Config.launcherKeybindsPrefix || "k";
        if (!searchText.startsWith(prefix)) return null;
        let rest = searchText.substring(prefix.length);
        if (rest.length > 0 && !/^\s/.test(rest)) return null;
        return { filter: rest.replace(/^\s+/, "") };
    ...
    property var filteredKeybinds: {
        if (!keybindsMatch) return [];
        let f = keybindsMatch.filter.toLowerCase().trim();
        // Optional leading [domain] token scopes to one source, e.g.
        // "[zsh] history" -> only zsh binds matching "history".
        let domainFilter = "";
        let m = f.match(/^\[([^\]]*)\]\s*/);
        if (m) {
            domainFilter = m[1].trim();
            f = f.substring(m[0].length).trim();
        }
        let out = [];
        for (let sec of keybindsData) {
            if (domainFilter !== "") {
                let dl = ((sec.domain || "") + " " + (sec.label || "")).toLowerCase();
                if (!dl.includes(domainFilter)) continue;
            }
            let groups = sec.groups || {};
            for (let g in groups) {
                for (let pair of groups[g]) {
                    let keys = pair[0], desc = pair[1];
                    let hay = (keys + " " + desc).toLowerCase();
                    let score = f === "" ? 1 : fuzzyScore(f, hay);
                    if (score > 0)
                        out.push({
                            keys: keys,
                            desc: desc,
                            group: (sec.domain || "") + (g ? " · " + g : ""),
                            score: score
                        });
                }
            }
        }
        if (f !== "") out.sort((a, b) => b.score - a.score);
    ...
    // Cache the parsed frecency blob; Config.launcherFrecencyData only changes
    // on launch (recordLaunch), so we avoid JSON.parse on every score lookup
    // (the sort comparator calls getFrecencyScore up to 2x per comparison).
    property var _frecencyCache: ({})
    property string _frecencyCacheRaw: " "  // sentinel: never equals real data
    function getFrecencyData() {
        if (Config.launcherFrecencyData === _frecencyCacheRaw)
            return _frecencyCache;
        try {
            _frecencyCache = JSON.parse(Config.launcherFrecencyData);
        } catch (e) {
            _frecencyCache = {};
        }
        _frecencyCacheRaw = Config.launcherFrecencyData;
        return _frecencyCache;
    }
    ...
    property var filteredResults: {
        ...
        if (keybindsMatch) {
            if (filteredKeybinds.length === 0)
                return [{ type: "info", message: "No keybinding matches" }];
            return filteredKeybinds.map(k => ({ type: "keybind", keys: k.keys, desc: k.desc, group: k.group }));
        }
        if (clipboardMatch) {
    ...
    function activateResult(item) {
        ...
        else if (item.type === "keybind") root.copyKeyChord(item.keys);
    ...
    IpcHandler {
        ...
        // Open the launcher directly in keybindings mode (Super+/).
        function openKeybinds(): void {
            let seed = (Config.launcherKeybindsPrefix || "k") + " ";
            if (root.visible && !root.closing && root.searchText === seed) {
                root.closing = true;
                return;
            }
                ...
                ColumnLayout {
                    ...
                    ListView {
                        ...
                        delegate: Rectangle {
                            ...
                            readonly property bool isKeybind: modelData.type === "keybind"
                                ...
                                ColumnLayout {
                                    ...
                                    Item {
                                        ...
                                        Text {
                                            ...
                                            anchors.right: parent.right
                                            anchors.verticalCenter: parent.verticalCenter
                                            visible: (resultItem.isFlatpak || resultItem.isPacman || resultItem.isSearch || resultItem.isFile || resultItem.isKeybind)
                                                && root.resultRepoLabel(resultItem.modelData) !== ""
                                            text: root.resultRepoLabel(resultItem.modelData)
                                            color: resultItem.isKeybind ? Config.lavender : Config.overlay0
                                            font.pixelSize: 11
                                            font.family: Config.fontFamily
                                        }
