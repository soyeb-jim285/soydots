...
QtObject {
    ...
    property string _hyprThemePath: _homeDir + "/.config/hypr/quickshell-theme.lua"

    // One hl.config() call, reused for both the persistent file and the live
    // hyprctl eval — Hyprland's Lua config merges repeated hl.config() calls.
    function _buildHyprConfigCall() {
        let b = blue.replace("#", "");
        ...
        return "hl.config({\n" +
            ...
            "        col = {\n" +
            "            active_border   = { colors = { \"rgba(" + b + "ee)\", \"rgba(" + l + "ee)\" }, angle = 45 },\n" +
            "            inactive_border = \"rgba(" + s + "aa)\",\n" +
            "        },\n" +
            "    },\n\n" +
            "    decoration = {\n" +
    ...
    function _doSyncHyprland() {
        ...
        let call = _buildHyprConfigCall();

        // Write persistent config file (survives reboot)
        _hyprWriteProc.command = ["bash", "-c",
            "cat > " + _hyprThemePath + " << 'HYPREOF'\n" +
            "-- Auto-generated by quickshell Config — do not edit manually\n\n" + call + "HYPREOF"];
        _hyprWriteProc.running = true;

        // Also apply live. hyprctl keyword is legacy-parser only; Lua uses eval.
        _hyprProc.command = ["hyprctl", "eval", call];
...
variant = "pill"'
    ...
    function _doSyncCursor() {
        ...
        let hyprConf = _homeDir + "/.config/hypr/hyprland.lua";
        ...
        //  4. sed the hl.env XCURSOR_*/HYPRCURSOR_* lines and the autostart
        //     hyprctl setcursor line in hyprland.lua so they persist across reboots
        let script =
            ...
            "sed -i " +
                "-e 's|^hl.env(\"XCURSOR_THEME\".*|hl.env(\"XCURSOR_THEME\", \"" + theme + "\")|' " +
                "-e 's|^hl.env(\"XCURSOR_SIZE\".*|hl.env(\"XCURSOR_SIZE\", \"" + size + "\")|' " +
                "-e 's|^hl.env(\"HYPRCURSOR_THEME\".*|hl.env(\"HYPRCURSOR_THEME\", \"" + theme + "\")|' " +
                "-e 's|^hl.env(\"HYPRCURSOR_SIZE\".*|hl.env(\"HYPRCURSOR_SIZE\", \"" + size + "\")|' " +
                "-e 's|hl.exec_cmd(\"hyprctl setcursor .*|hl.exec_cmd(\"hyprctl setcursor " + theme + " " + size + "\")|' " +
                hyprConf;
